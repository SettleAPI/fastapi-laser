...
class Migration:
    ...
    @staticmethod
    async def current(db_session: sqlalchemy.ext.asyncio.AsyncSession) -> Optional[str]:
        logger.info("Getting current online database revision...")

        try:
            result = await db_session.execute("SELECT version_num FROM alembic_version")
            current_version = result.scalars().first()
            return current_version
        except sqlalchemy.exc.DatabaseError:
            return None

    async def is_current_head(self, db_session: sqlalchemy.ext.asyncio.AsyncSession) -> bool:
        return self.head() == await Migration.current(db_session)
