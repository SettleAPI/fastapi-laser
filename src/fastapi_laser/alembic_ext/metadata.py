...
class EnumToColumnsMap:
    ...
    def diff(self, other: "EnumToColumnsMap") -> DiffDepth1:
        return DiffDepth1(
            ...
            set(other.members) - set(self.members),
        )
                    ...
                    if not result.get(column_type_name):
                        ...
                        result[column_type_name] = EnumToColumnsMap(
                            ...
                            member_removal_rules=getattr(
                                ...
                                "member_removal_db_migration_rules",
                                dict(),
                            ),
                        )
...
def get_migration_config_items(config: Config, target_metadata: MetaData) -> dict:
    ...
    return dict(
        template_args=dict(
            enum_args_variable_assignments=enum_args_variable_assignments,
...
def process_revision_directives_factory(
    target_enum_to_column_maps: EnumToColumnsMapCollection,
    online_enum_to_column_maps: EnumToColumnsMapCollection,
) -> Callable:
...
def get_enum_names_by_op_kind(ops_list: Collection[OpContainer]) -> dict[str, set[str]]:
    result = dict(
        add_column=set(),
        remove_column=set(),
        add_table_column=set(),
        remove_table_column=set(),
    )
    for op in ops_list:
